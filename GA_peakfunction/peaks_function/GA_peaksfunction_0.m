...
f =@(x,y) 3*(1-(x-3)).^2.*exp(-((x-3).^2) - (y-2).^2) ...
    - 10*((x-3)/5 - (x-3).^3 - (y-3).^5).*exp(-(x-3).^2-(y-3).^2) ...
    - 1/3*exp(-(x-2).^2 - (y-3).^2) ;
...
% Generate all points
for i=1:p
    X(1,i)=rand();Y(1,i)=rand();
    X(1,i)=(0.5-X(i))*6+3;Y(1,i)=(0.5-Y(i))*6+3;
    Z(1,i)=f(X(1,i),Y(1,i));
end
...
% To satisfy x,y in [0,6];
fprintf("Constrain condition\n")
for i=1:p
    if x(k+1,i)>6
        x(k+1,i)=6-rand();
        fprintf('%2d\t',i);fprintf('%4.3f',x(k+1,i));fprintf("\n");
    end
    if x(k+1,i)<0
        x(k+1,i)=rand();
        fprintf('%2d\t',i);fprintf('%4.3f',x(k+1,i));fprintf("\n");
    ...
    if y(k+1,i)>6
        y(k+1,i)=6-rand();
        fprintf('%2d\t',i);fprintf('%4.3f',y(k+1,i));fprintf("\n");
    end
    if y(k+1,i)<0
        y(k+1,i)=rand();
        fprintf('%2d\t',i);fprintf('%4.3f',y(k+1,i));fprintf("\n");
...
for j=1:k+1
    figure
    xxx = linspace(-3, 3, 30);		% in x-axis,[-3,3] choose 30 pts. 
    yyy = linspace(-3, 3, 30);		% in y-axis,[-3,3] choose 30 pts. 
    [xxx,yyy] = meshgrid(xxx, yyy);	% xxx,yyy are all array.
    zzz = f1(xxx,yyy);	% zzz same as xxx,yyy type
    contour(xxx, yyy, zzz, 20);hold on
    colormap('default');				% colar default
    xlim([-3,3]);ylim([-3,3]);xlabel('x');ylabel('y');
    title('Peaks Function')
    plot3(x_plot(j,:),y_plot(j,:),z(j,:),'o','MarkerFaceColor','b');
end
...
X1_plot=X1-3;Y1_plot=Y1-3;
A=1:k+1;

figure
plot(A,X1_plot,'-o',A,Y1_plot,'-.>',A,Z1,':+');title('The best value after the GA method in every step');
xlabel('step(s)');ylabel('value');legend('x','y','z');

figure
for i=1:p
    plot(1:k+1,z(:,i),'-*');title('All value in GA method');xlabel('Steps');ylabel('f(x1,x2)');
    hold on
end
